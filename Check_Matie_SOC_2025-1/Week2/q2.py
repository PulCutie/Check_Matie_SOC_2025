...
class History:
    def __init__(self, num_boards, history=None):
        """
    ...
    def get_valid_actions(self):
        ...
        for board in range(len(boards)):
            if not self.is_board_win(boards[board]):
                #print(boards[board])
                for i in range(len(boards[board])):
                    if boards[board][i]=='0':
                        actions.append(board*9 + i)
        return actions
...
def sort_actions(actions):
    ...
    for i in actions:
        if i%9==4:
            ...
            other.append(i)
    return centre+other2+other

def alpha_beta_pruning(history_obj, alpha, beta, max_player_flag):
    global visited_histories_list, boardstate_dict
    if history_obj.get_boards_str() in boardstate_dict:
        return boardstate_dict[history_obj.get_boards_str()]
    ...
    if max_player_flag:
        ...
        #for move in sort_actions(history_obj.get_valid_actions()):
            newhistory = History(history = history_obj.history + [move], num_boards=history_obj.num_boards)
            val = alpha_beta_pruning(newhistory, alpha, beta, False)
            best = max(best, val)
            alpha = max(alpha, val)
            if beta<=alpha:
                break
        ...
        #for move in sort_actions(history_obj.get_valid_actions()):
            newhistory = History(history = history_obj.history + [move], num_boards=history_obj.num_boards)
            val = alpha_beta_pruning(newhistory, alpha, beta, True)
            best = min(best, val)
            beta = min(beta, val)
            if beta<=alpha:
                break
    
    boardstate_dict[history_obj.get_boards_str()] = best
    #print(history_obj.history, best)
    return best
...
if __name__ == "__main__":
    ...
    value, visited_histories = solve_alpha_beta_pruning(History(num_boards=2, history=[],), -math.inf, math.inf, True)
